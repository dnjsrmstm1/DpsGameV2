...
function 강화시도(단계: number, 스텟: 강화스텟, 외부p1: number = 0, 보석p: 보석p타입 = 빈보석p, 초월s?: 초월스텟타입, 일반p1: number = 0): number {
  ...
  // 40~43강: 특수 확률 + 가산1~3강 스텟 + 외부보너스 (궁극 등)
  if (단계 >= 40 && 단계 <= 43) {
    const 가산 = (스텟.가산1강 + 스텟.가산1강2 + 스텟.가산2강 + 스텟.가산2강2 + 스텟.가산3강 + 스텟.가산3강2) * 0.001
    const bonus = (스텟.특수강화 + 스텟.특수강화2) * 0.001 + 가산 + 외부p1
    return r < Math.min(0.95, bonus) ? 1 : 0
  }
  // 1~25강: base + 강화확률 스텟/보석, 단 합산 상한캡(throughput 폭주 방지 — 20단계 복리라 작은값도 큼)
  if (단계 <= 25) {
    const 보너스 = Math.min(초반보너스캡,
      (스텟.가산1강 + 스텟.가산1강2) * 0.001
      + 외부p1 + 일반p1)
    const s1 = Math.min(0.95, base + 보너스)
  ...
  // 26~39강: 스텟/외부 보너스 적용 (특수강화는 일반확률 미적용 — 특수확률 전용)
  if (단계 >= 38 && 단계 <= 39) {
  ...
  const p1 = Math.min(0.95, base       + (스텟.가산1강 + 스텟.가산1강2) * 0.001 + 외부p1 + 일반p1)
  if (r < p3) return 3
  if (r < p3 + p2) return 2
...
// 판매 획득 상세 (정보 패널용) — 기본획득(확정) + 확률획득(드랍률)
function 판매상세(lv: number): { 기본: string[]; 확률: string[] } {
  const 기본: string[] = [], 확률: string[] = []
  if (lv < 15) return { 기본: ['보상 없음'], 확률: [] }
  if (lv === 51) return { 기본: ['판매 불가'], 확률: [] }
  if (lv <= 44) { 기본.push(`⭐ 일반경험치 ${숫자포맷(판매일반XP표[lv] ?? 0)}`); return { 기본, 확률 } }
  switch (lv) {
    case 45: 기본.push('🔷 무색 1'); 확률.push('🔷 무색 1만 (1.5%)', '💰 크레딧 200만 (5%)', '💎 각성석 1 (0.05%)', '💰 크레딧 32.2억 (0.002%)'); break
    case 46: 기본.push('🔷 무색 4'); 확률.push('🔷 무색 4만 (0.55%)', '💰 크레딧 300만 (5%)', '💎 각성석 1 (0.075%)', '💰 크레딧 32.2억 (0.002%)'); break
    case 47: 기본.push('🔷 무색 7'); 확률.push('🔷 무색 7만 (0.47%)', '💰 크레딧 400만 (5%)', '💎 각성석 1 (0.16%)', '💰 크레딧 32.2억 (0.002%)'); break
    case 48: 기본.push('🔷 무색 10'); 확률.push('🔷 무색 10만 (0.99%)', '💰 크레딧 500만 (5%)', '💎 각성석 1 (0.5%)', '💰 크레딧 32.2억 (0.002%)'); break
    case 49: 기본.push('🔷 무색 322'); 확률.push('🔷 무색 100만 (1.5%)', '🔷 무색 10만 (15%)', '💎 각성석 7 (1%)', '💰 크레딧 1000만 (30%)', '💰 크레딧 32.2억 (0.022%)'); break
    case 50: 기본.push('🔷 무색 3,222'); 확률.push('🔷 무색 400만 (15%)', '💎 각성석 75 (5%)', '💰 크레딧 1억 (35%)', '💰 크레딧 322.2억 (0.05%)'); break
    case 52: 기본.push('🔮 크리스탈조각 1', '⭐ 초월XP 500'); 확률.push('🌟 노말박스 (3%)'); break
    case 53: 기본.push('🔮 크리조각 12', '⭐ 초월XP 8,000'); 확률.push('🌟 레어박스 (1%)', '🌟 노말박스 (7%)'); break
    case 54: 기본.push('🔮 크리조각 300', '⭐ 초월XP 10만'); 확률.push('🌟 갤럭시 (0.002%)', '🌟 유니크 (0.1%)', '🌟 레어 (4%)', '🌟 노말 (18%)'); break
    case 55: 기본.push('🔮 크리조각 2,000', '⭐ 초월XP 140만'); 확률.push('🌟 갤럭시 (0.01%)', '🌟 유니크 (1%)', '🌟 레어 (30%)', '🌟 노말 (50%)'); break
    case 56: 기본.push('🔮 크리조각 1만', '⭐ 초월XP 2,500만'); 확률.push('🌟 갤럭시 (0.1%)', '🌟 유니크 (5%)', '🌟 레어 (70%)'); break
    case 57: 기본.push('💠 응무조 1'); 확률.push('💠 응무조 +10 (30%)', '🌟 퀘이사 (0.002%)', '🌟 갤럭시×50 (2%)', '🌟 유니크×100 (10%)'); break
    case 58: 기본.push('💠 응무조 15'); 확률.push('💠 응무조 +200 (30%)', '🌟 퀘이사×10 (0.002%)', '🌟 퀘이사 (0.1%)', '🌟 갤럭시×1000 (5%)', '🌟 유니크×2000 (10%)'); break
    case 59: 기본.push('💠 응무조 300'); 확률.push('💠 응무조 +1000 (50%)', '🌟 퀘이사×50 (0.05%)', '🌟 퀘이사×3 (1%)'); break
    case 60: 기본.push('💠 응무조 1억', '🎯 60강 희생(목표 카운트 +1)'); 확률.push('🌟 오리진박스 (50%)', '🔱 자각보주 1 (25%)', '🌌 은하조각 1 (25%)'); break
  }
  return { 기본, 확률 }
}
...
function 보석보너스합산(b: 보석목록) {
  ...
  const 경험배수 = Math.pow(1.003, b.경험보석)
...
export default function App() {
  ...
  const [누적60강희생, set누적60강희생] = useState(0)  // 🎯 게임 목표: 60강 누적 20만 희생(판매)
  ...
  // 유닛 1000마리 초과 시 자동으로 내부계산모드 ON (렉 방지)
  useEffect(() => { if (마린들.length > 1000 && !내부계산모드Ref.current) set내부계산모드(true) }, [마린들.length])
  ...
  const 강화단계정보 = (lv: number): { 타입: string; 확률: string[]; 판매: string; 구입: number; 공격력: number; 공속: string; dps: number; 판매획득: string; 내역: string[] } => {
    ...
    if (lv >= 60) { 타입 = 'MAX'; 확률 = ['더 이상 강화 불가'] }
      ...
      타입 = '특수 확률'; 확률 = [`성공 ${pct(특수가산 + g + 외부)}  (특수강화+가산1~3강)`]
    }
    else if (lv >= 38) {
      const base = 강화기본확률(lv)
      const g = (s.가산1강 + s.가산1강2 + s.가산2강 + s.가산2강2 + s.가산3강 + s.가산3강2) * 0.001
      ...
      const base = 강화기본확률(lv)
      const p3 = base / 100 + (s.가산3강 + s.가산3강2) * 0.001
      const p2 = base / 10 + (s.가산2강 + s.가산2강2) * 0.001
      const p1 = base + (s.가산1강 + s.가산1강2) * 0.001 + 외부 + 일반보
    ...
    if ((lv < 49) || (lv >= 51 && lv <= 59)) {  // 49강(고정)·50강(초월시도)·60강 제외
      ...
      else if (lv <= 43) { push('⚡ 스텟(특수강화)', 특수가산); push('📊 스텟(가산1~3강)', (s.가산1강 + s.가산1강2 + s.가산2강 + s.가산2강2 + s.가산3강 + s.가산3강2) * 0.001) }
    ...
    AsyncStorage.getItem(저장키).then(json => {
      if (json) {
        try {
          ...
          if (typeof d.누적60강희생 === 'number') set누적60강희생(d.누적60강희생)
          // 오프라인 보상
  ...
  // 렌더
  // ============================================
  // 정적빌드 HTML과 클라이언트 초기 렌더 불일치(React #418) 방지 — 마운트 후에만 렌더
  ...
  return (
    ...
    >
      <Text style={styles.title}>DPS 강화하기 ⚔️ RTS  <Text style={{ fontSize: 11, color: '#7ed957' }}>BUILD C16</Text></Text>
      ...
      {재화패널열림 && (
        <View style={[styles.currencyPanel, 패널정렬]}>
          ...
          {/* 시간당 ExP 패시브 정보 */}
          <View style={{ backgroundColor: '#0d1526', borderRadius: 6, padding: 8, marginTop: 8 }}>
          ...
          <Text style={{ color: '#888', fontSize: 10, marginTop: 6 }}>💡 ExP 뽑기·쿠폰은 🛒 상점에서</Text>
          </ScrollView>
        </View>
      )}
      ...
      {도움말패널열림 && (
        <View style={[styles.currencyPanel, 패널정렬]}>
          ...
          <ScrollView style={{ maxHeight: 필드_H - 120 }}>
            {[
              ['🎯 최종 목표', `60강 마린을 판매소에 넣으면 '희생'이 일어난다. 누적 20만 마리의 60강을 희생하는 것이 이 게임의 목표! (현재 ${숫자포맷(누적60강희생)} / 200,000)`],
              ['⚒️ 강화', '마린 선택 → 강화소 또는 자동강화. 성공 시 +1강(저강은 +2/+3도). 실패 시 마린 파괴(6~49강은 파괴방지 적용). 강화마다 자원 소모. 50→51강은 초월 시도, 51~59강은 외부보너스·초월스텟 기반.'],
              ['🔢 강화 확률 구조', '1~25강: base 확률 + 스텟/보석(합산 상한캡). 26~39강: 가산1~3강·특수강화·보석·보주 적용. 40~43강: 특수확률(특수강화+가산1~3강). 44~48강: 개별확률+보석. 49강: 고정 0.5%. 60강 = MAX.'],
              ['🐺 사냥터/DPS', '마린을 사1/사2/사3에 두면 자동 공격 → DPS 측정 → 자원 생성. 티어↑ 단가↑. 사3(허수광산)은 크레딧도 함께. DPS 높을수록 자원배수↑.'],
              ['🛒 판매/획득', '판매소에 넣으면 강별 재화 획득(정보 패널에서 강별 상세 비율 확인). 15~44강=경험치, 45~50강=무색조각+확률드랍, 51강+=초월경험·조각·박스, 60강=희생.'],
              ['🏪 자동화', '자동강화/자동판매/자동구입 ON. 구입은 50강까지. 촉진(보석·일반/초월스텟·고유단수)으로 틱당 생산량↑.'],
              ['💰 재화', '미네랄(자원·강화·구입) / 크레딧(고유유닛·판매비) / 무색조각(보석) / 응무조(보주) / 각성석·크리스탈·상급조각 / ExP(뽑기·시간당 지급) / 일반P·초월P(스텟).'],
              ['⭐ 시간당 ExP', '로그인(온라인) 중 20분마다 ExP 자동 지급(최고강 비례). 재화 패널에서 다음 지급까지 시간 확인. 새로고침/로그아웃해도 타이머 유지.'],
              ['💠 보석·🔮 보주·🏅 명칭·🦸 고유유닛', '강화확률·자원·DPS·파괴방지 등 보너스. 무색조각·응무조·각성석 등으로 구입/강화. 고유유닛은 모든 강화 MAX 시 각성석으로 단수업.'],
              ['🌀 초월', '30만 레벨 도달 후 해금. 51강+ 강화·고강 판매로 초월경험, 초월스텟 분배.'],
              ['🌟 환생', '누적 50강+ 생산 1마리당 ExP 보상. 마린 초기화, 스텟/보석/보주/초월/크레딧 유지.'],
              ['👤 로그인/동기화', '로그인 필수(미로그인 시 게임 정지). 진행도는 클라우드 자동 저장(2분)·기기 간 공유. 1기기만 동시 접속.'],
              ['🧹 리스펙/초기화', '스텟/보석/보주/초월 카테고리별 전액 환불 리셋. 전체 초기화는 하단 버튼.'],
              ['👆 마린 선택', '마린을 탭하거나 드래그(여러 마리)해서 선택.'],
            ].map(([제목, 내용]) => (
            ...
            return (
              <>
                ...
                <ScrollView style={{ maxHeight: 필드_H - 120 }}>
                  ...
                  <Text style={styles.currencySection}>💰 판매 획득 상세</Text>
                  <Text style={{ color: '#cfd6e4', fontSize: 14 }}>판매 비용: {info.판매}</Text>
                  {(() => {
                    const d = 판매상세(정보단계)
                    return (
                      <>
                        <Text style={{ color: '#7ed957', fontSize: 12, marginTop: 4, fontWeight: 'bold' }}>📦 기본 획득 (확정)</Text>
                        {d.기본.length ? d.기본.map((l, i) => <Text key={i} style={{ color: '#cfd6e4', fontSize: 13, lineHeight: 18 }}>· {l}</Text>) : <Text style={{ color: '#888', fontSize: 11 }}>· 없음</Text>}
                        {d.확률.length > 0 && (
                          <>
                            <Text style={{ color: '#f5a623', fontSize: 12, marginTop: 5, fontWeight: 'bold' }}>🎲 확률 획득 (드랍률)</Text>
                            {d.확률.map((l, i) => <Text key={i} style={{ color: '#cfd6e4', fontSize: 13, lineHeight: 18 }}>· {l}</Text>)}
                          </>
                        )}
                      </>
                    )
      ...
      <View style={styles.tabBar}>
        ...
        >
          <Text style={styles.tabText}>🏠 ({베이스마린들.length}) [{마린들.length}/10000]</Text>
        </TouchableOpacity>
        <TouchableOpacity
          ...
          <ScrollView style={{ maxHeight: 필드_H - 120 }}>
            ...
            {스텟탭 === '보석' && (() => {
              const 보석정보in: { 종류: 보석타입; 이모지: string; 설명: string; 효과: string }[] = [
                ...
                { 종류: '경험보석',이모지: '📗', 설명: '경험의 보석', 효과: '경험치 ×1.003/회 (복리)' },
