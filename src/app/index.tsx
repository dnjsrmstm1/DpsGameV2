...
const 보주구입비용: Record<보주타입, number> = {
  ...
  풍성: 1e19, 부유: 1e22, 무엄: 1e23, 공명: 1e20,
}
...
type 보석타입 = '하급' | '중급' | '상급' | '특급' | '고급' | '재물' | '경험보석' | '보호보석' | '궁극' | '수호' | '초월보석' | '인내' | '강타' | '자동화' | '채광'
type 보석목록 = { [K in 보석타입]: number }
const 초기보석: 보석목록 = { 하급: 0, 중급: 0, 상급: 0, 특급: 0, 고급: 0, 재물: 0, 경험보석: 0, 보호보석: 0, 궁극: 0, 수호: 0, 초월보석: 0, 인내: 0, 강타: 0, 자동화: 0, 채광: 0 }
...
function 보석보너스합산(b: 보석목록) {
  ...
  const 자원배수추가 = (Math.pow(1.1, b.재물) * Math.pow(1.05, b.채광)) - 1
...
// 보주 효과표 — 효과 미정 (사용자 추후 제공). 일단 모두 '미정' / 값 0
const 보주효과표: Record<보주타입, { 타입: string; 값: number; 설명: string }> = Object.fromEntries(
  보주종류목록.map(k => [k, { 타입: '미정', 값: 0, 설명: '효과 미정' }])
) as Record<보주타입, { 타입: string; 값: number; 설명: string }>
...
// 명칭 크리스탈 패시브 보너스 (장착된 크리스탈만 적용)
function 명칭크리스탈보너스(m: 명칭크리스탈목록, eq: (keyof 명칭크리스탈목록)[]) {
  const eqSet = new Set(eq)
  const v = (k: keyof 명칭크리스탈목록): number => eqSet.has(k) ? (m[k] as number) : 0
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const 명칭보너스 = 명칭크리스탈보너스(명칭크리스탈Ref.current, 장착크리스탈Ref.current)
      ...
      // 사냥터 곱셈 보너스 — 복리: 1.1^재물 × 1.1^각성
      const _재물 = 보석Ref.current.재물 || 0
      const _각성 = 각성의보석Ref.current || 0
      const 사냥터곱셈 = Math.pow(1.1, _재물) * Math.pow(1.1, _각성)
      ...
      // 보석 각성 획득량 보너스 (장착된 각성/노랑/우주 크리스탈만 적용)
  ...
  function 보주구입(종류: 보주타입, 수량: number) {
    ...
    set보주(prev => ({ ...prev, [종류]: prev[종류] + 수량 }))
  }
  function 보주최대구입(종류: 보주타입) {
    const 단가 = 보주구입비용[종류]
    const 가능수 = Math.floor(응무조Ref.current / 단가)
          ...
          <ScrollView style={{ maxHeight: 360 }}>
            ...
            {스텟탭 === '보석' && (() => {
              const 보석정보in: { 종류: 보석타입; 이모지: string; 설명: string; 효과: string }[] = [
                ...
                { 종류: '재물',    이모지: '💰', 설명: '재물의 보석', 효과: '자원 ×1.1/회 (복리)' },
      ...
      {명칭크리스탈패널열림 && (() => {
        ...
        const 크리스탈목록표: Record<크탭, 크정보[]> = {
          노말: [
            ...
            { 키: '절약',  이름: '절약의 크리스탈',  비용: 50,  색상: '#f5a623', 효과: `판매보상 ×${Math.pow(1.05, 명칭크리스탈.절약).toFixed(2)} (회당×1.05)`, 구현: true },
            { 키: '총명',  이름: '총명의 크리스탈',  비용: 50,  색상: '#1abc9c', 효과: `무색조각 ×${Math.pow(1.5, 명칭크리스탈.총명).toFixed(2)} (회당×1.5)`,  구현: true },
          ...
          레어: [
            { 키: '홍색',   이름: '홍색의 크리스탈',  비용: 100, 색상: '#e74c3c', 효과: `개별확률 +${(명칭크리스탈.홍색 * 1).toFixed(0)}%`, 구현: true },
            { 키: '주황',   이름: '주황의 크리스탈',  비용: 100, 색상: '#e67e22', 효과: `판매 ×${Math.pow(1.05, 명칭크리스탈.주황).toFixed(2)} + 무색 ×${Math.pow(1.5, 명칭크리스탈.주황).toFixed(2)}`, 구현: true },
            ...
            { 키: '보라',   이름: '보라의 크리스탈',  비용: 100, 색상: '#9b59b6', 효과: `개별확률 +${(명칭크리스탈.보라 * 1).toFixed(0)}% + 판매 ×${Math.pow(1.05, 명칭크리스탈.보라).toFixed(2)}`, 구현: true },
            { 키: '하늘색', 이름: '하늘색 크리스탈',  비용: 100, 색상: '#5dade2', 효과: `무색 ×${Math.pow(1.5, 명칭크리스탈.하늘색).toFixed(2)} + 각성보석 (준비중)`, 구현: true },
            { 키: '무색명칭', 이름: '무색의 크리스탈', 비용: 100, 색상: '#bdc3c7', 효과: `초월확률 +${(명칭크리스탈.무색명칭 * 0.2).toFixed(1)}% (회당+0.2%)`, 구현: true },
          ],
          유니크: [
            { 키: '흑색', 이름: '흑색의 크리스탈', 비용: 500, 색상: '#7f8c8d', 효과: `무색 ×${Math.pow(2, 명칭크리스탈.흑색).toFixed(2)} + 판매 ×${Math.pow(1.10, 명칭크리스탈.흑색).toFixed(2)} + 초월확률 +${(명칭크리스탈.흑색 * 0.5).toFixed(1)}%`, 구현: true },
            { 키: '백색명칭', 이름: '백색의 크리스탈', 비용: 500, 색상: '#ecf0f1', 효과: `개별확률 +${(명칭크리스탈.백색명칭 * 2).toFixed(0)}% + 파괴방지 +${(명칭크리스탈.백색명칭 * 0.3).toFixed(1)}% + 초월경험치 (준비중)`, 구현: true },
          ],
          갤럭시: [
            { 키: '우주', 이름: '우주의 크리스탈', 비용: 2000, 색상: '#a855f7', 효과: `개별확률 +${(명칭크리스탈.우주 * 5).toFixed(0)}% + 파괴방지 +${(명칭크리스탈.우주 * 0.5).toFixed(1)}% + 초월확률 +${(명칭크리스탈.우주 * 1).toFixed(0)}% + 무색 ×${Math.pow(3, 명칭크리스탈.우주).toFixed(2)} + 판매 ×${Math.pow(2, 명칭크리스탈.우주).toFixed(2)}`, 구현: true },
          ],
          퀘이사: [
